...
ClientsList()
{
    sockaddr_in data = SenderAddr;

    // Count if the client exists
    bool already_exists = false;
    for (int i = 0; i < clients.size(); ++i) {
        if (clients[i].sin_addr.s_addr == SenderAddr.sin_addr.s_addr) {
                already_exists = true;
            break;
        }
    }
...
receiveFrom()
{
    ...
    if (iResult > 0) {
        ...
        std::cout << "From the client with IP " << ip << " port "
            << SenderAddr.sin_port << ".";
        ClientsList();
        aux++;
...
send_to()
{
    ...
    iResult = sendto(RecvSocket, SendBuf, BufLen, 0, (SOCKADDR*)&SenderAddr,
        sizeof(SenderAddr));
